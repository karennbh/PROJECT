...
@section('page_title', 'Form Pengajuan Pembelian Barang Kantor')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2">
            <div class="modern-card p-6 md:p-8">
                <form action="{{ route('pembelian.store') }}" method="POST" enctype="multipart/form-data" id="pembelianForm" class="space-y-5" novalidate>
                    @csrf
                            ...
                            <div class="item-row animate-main" data-index="0">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                                    <div>
                                        ...
                                        <input type="text" name="items[0][nama_barang]" class="nama-barang-input modern-input px-3 py-2 {{ $errors->has('items.0.nama_barang') ? 'is-invalid' : '' }}" placeholder="Nama Barang" value="{{ old('items.0.nama_barang') }}" minlength="3" maxlength="50" required>
                                        <p class="nama-barang-err field-error {{ $errors->has('items.0.nama_barang') ? '' : 'hidden' }}">{{ $errors->first('items.0.nama_barang') ?: 'Nama barang wajib diisi.' }}</p>
                                    </div>
                                    <div>
        ...
        <div class="space-y-6">
            <div class="modern-card p-5">
                ...
                <div id="filterForm" class="mb-6 p-4 bg-blue-50/50 rounded-2xl border border-blue-100 {{ request()->anyFilled(['tanggal_dari', 'tanggal_sampai', 'status']) ? '' : 'hidden' }}">
                    <form method="GET" action="{{ route('pembelian.index') }}" class="space-y-3">
                        ...
                        <div class="flex gap-2 pt-1">
                            ...
                            <a href="{{ route('pembelian.index') }}" class="px-3 py-2.5 bg-white text-slate-400 rounded-lg text-sm border border-slate-200">Reset</a>
                        </div>
                    </form>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="p-4 rounded-2xl border border-slate-50 bg-slate-50/30 hover:bg-white hover:border-blue-100 transition-all group animate-main">
                        <div class="flex justify-between items-start mb-2">
                            <div class="space-y-1">
                                <p class="text-xs font-black text-blue-500 uppercase tracking-widest">{{ $item->kategori_barang }}</p>
                                <h4 class="text-xs font-bold text-slate-800 group-hover:text-blue-600 transition-colors">
                                    {{ $item->nama_barang }}
                                </h4>
                                <p class="text-xs font-bold text-slate-700">x{{ $item->jumlah }} Unit</p>
                                <p class="text-sm text-blue-600 font-bold">Rp {{ number_format($item->sub_total, 0, ',', '.') }}</p>
                            </div>
                            @php
                        ...
                        <div class="flex items-center justify-end text-xs text-slate-500 pt-3 border-t border-slate-100/50">
                            <span class="flex items-center gap-1"><i class="far fa-calendar"></i> {{ \Carbon\Carbon::parse($item->tanggal_pengajuan)->format('d/m/y') }}</span>
                        </div>
                    </div>
                    @empty
                ...
                <div class="mt-6 pt-4 border-t border-slate-100">
                    <a href="{{ route('pembelian.riwayat') }}" class="block text-center py-3 bg-sky-400 hover:bg-sky-500 text-white rounded-xl text-xs font-bold shadow-lg shadow-sky-400/25 transition-all">
                        Buka Semua Riwayat
                    </a>
                </div>
                @endif
...
function attachHargaEvents(displayInput) {
    ...
    displayInput.validateHarga = function () {
        ...
        const row = displayInput.closest('.item-row');
        const jumlah = parseInt(row?.querySelector('.input-jumlah')?.value || '0', 10);
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    itemsContainer.addEventListener('input', function (e) {
        if (e.target.classList.contains('input-jumlah')) {
            ...
            e.target.closest('.item-row')?.querySelector('.input-harga-display')?.validateHarga?.();
            calculateGrandTotal();
        }
