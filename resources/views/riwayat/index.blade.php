...
@section('page_title', 'Riwayat Keseluruhan Aktivitas')
...
<div class="space-y-6 pb-8 animate-main max-w-[1400px]">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div class="flex items-center gap-2.5">
            <a href="{{ route('anggota.dashboard') }}" class="text-sm font-semibold text-slate-600 bg-white px-4 py-2.5 rounded-lg shadow-sm border border-slate-200 hover:bg-slate-50 transition-all">
    ...
    <div class="modern-card p-5">
        ...
        <form method="GET" action="{{ route('riwayat.index') }}" class="grid grid-cols-1 md:grid-cols-5 gap-4">
            <div>
                ...
                <select name="modul" class="modern-input px-3 py-2 bg-white" onchange="this.form.submit()">
                    ...
                    <option value="pemakaian" {{ request('modul') == 'pemakaian' ? 'selected' : '' }}>Pemakaian</option>
            ...
            <div>
                ...
                <input type="date" id="tanggal_dari" name="tanggal_dari" value="{{ request('tanggal_dari') }}" class="modern-input px-3 py-2" onchange="syncTanggalSampai(); this.form.submit();">
            </div>
            <div>
                <label class="modern-label">Tanggal Sampai</label>
                <input type="date" id="tanggal_sampai" name="tanggal_sampai" value="{{ request('tanggal_sampai') }}" min="{{ request('tanggal_dari') }}" class="modern-input px-3 py-2" onchange="this.form.submit()">
            </div>
            <div class="flex items-end">
                <a href="{{ route('riwayat.index') }}" class="w-full text-center px-4 py-2.5 bg-slate-100 text-slate-700 rounded-xl text-sm font-bold hover:bg-slate-200 transition-all">
                    Reset Filter
                </a>
            </div>
        </form>
    ...
    <div class="modern-card p-5">
        <div class="mb-4 flex items-center justify-between">
            <h3 class="panel-title">
                Daftar Aktivitas ({{ $riwayat->total() }} Item Ditemukan)
            </h3>
        </div>
                            ...
                            <div class="flex-1">
                                <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-2 mb-1">
                                    <div class="space-y-1">
                                        <div class="flex flex-wrap items-center gap-2">
                                            <span class="text-xs font-black px-2.5 py-1 rounded-full border {{ $item['modul_color'] }} uppercase">
                                                {{ $item['modul_label'] }}
                                            </span>
                                        ...
                                        <h4 class="text-sm font-bold text-slate-800">{{ $item['judul'] }}</h4>
                                    </div>
                                    <span class="text-sm font-bold px-3 py-1.5 rounded-full border {{ $item['status_class'] }} uppercase">
                                        {{ $item['status_label'] }}
                                    </span>
                                </div>

                                <p class="text-xs text-slate-600 mb-2">
                                    <span class="font-semibold text-slate-400">Keterangan:</span> {{ $item['detail'] }}
                                </p>
                                ...
                                <div class="flex flex-wrap items-center gap-3 text-sm text-slate-500">
                                    ...
                                    <span class="flex items-center gap-1">
                                        <span class="font-semibold text-slate-700">{{ $item['tanggal_label'] }}:</span>
