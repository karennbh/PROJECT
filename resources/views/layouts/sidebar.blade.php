...
<head>
    <meta charset="UTF-8">
    <title>@yield('title', 'Pengelolaan Aset & BHP')</title>
...
<body>
    <div id="layout-root" class="min-h-screen flex">
        ...
        <aside id="sidebar" class="fixed top-0 left-0 h-screen flex flex-col z-50 shadow-2xl overflow-hidden">
            <div class="h-[80px] flex items-center gap-3 px-6 pt-2 flex-shrink-0">
                @include('partials.app-logo', ['theme' => 'dark'])
            </div>
                ...
                <div class="pt-4 pb-4"> 
                    ...
                    <nav class="relative z-10 space-y-1.5">
                        ...
                        @foreach($menus as $menu)
                            ...
                            <a href="{{ $menu['url'] }}" class="nav-item group gap-4 {{ $isActive ? 'active' : '' }}">
                                <div class="nav-dot"></div>
                                <div class="menu-text flex flex-col">
                                    <span class="menu-title">{{ $menu['label'] }}</span>
                                    <span class="menu-desc {{ $isActive ? 'text-brand-dark/70' : 'text-brand-light/40' }} hidden group-hover:block transition-all">{{ $menu['desc'] }}</span>
                                </div>
                            </a>
                        @endforeach
                    </nav>
        ...
        <div class="main-content flex-1 flex flex-col min-h-screen relative z-0">
            <header id="topbar" class="fixed top-0 right-0 h-[70px] bg-white/80 backdrop-blur-md shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)] border-b border-brand-light z-40 flex items-center justify-between px-8">
                <div class="flex items-center gap-4">
                    ...
                    <h1 class="text-base font-bold text-brand-navy tracking-tight">@yield('page_title', 'Overview Dashboard')</h1>
                </div>
                ...
                <button onclick="toggleProfilePanel()" class="flex items-center gap-3 group p-1.5 pr-4 rounded-full hover:bg-brand-light/50 transition-all border border-transparent hover:border-brand-light">
                    <div class="text-right hidden sm:block">
                        <p class="text-sm font-bold text-brand-navy group-hover:text-brand-blue transition-colors">{{ Auth::user()->name ?? 'Administrator' }}</p>
                        <p class="text-xs font-medium text-slate-400">{{ Auth::user()->user_group ?? 'Super Admin' }}</p>
                    </div>
                    <div class="relative">
            ...
            <div class="flex-1 overflow-y-auto custom-scrollbar">
                ...
                <div class="px-6 relative z-10">
                    <div class="relative -mt-10 mb-6 flex flex-col items-center">
                        ...
                        <h2 class="mt-3 text-lg font-bold text-brand-dark">{{ Auth::user()->name ?? 'Administrator' }}</h2>
                        <span class="text-xs text-slate-500">{{ Auth::user()->username ?? Auth::user()->name ?? 'admin' }}</span>
                    </div>
    ...
    @include('partials.tab-auth-guard')
    @include('partials.anggota-session-timeout', [
        'timeoutMs' => (int) config('session.lifetime', 45) * 60 * 1000,
        'logoutUrl' => route('logout'),
        'loginUrl' => route('login'),
    ])
    @includeWhen(!request()->routeIs('anggota.dashboard'), 'partials.feature-back-guard', ['fallbackUrl' => route('anggota.dashboard')])
