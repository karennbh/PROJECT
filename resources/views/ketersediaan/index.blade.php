...
@section('title', 'Mencari Ketersediaan Barang')
@section('page_title', 'Ketersediaan Aset & BHP')
...
<div class="space-y-8 pb-10 animate-main max-w-[1400px]">
    ...
    <div class="relative overflow-hidden rounded-2xl bg-slate-900 shadow-xl shadow-slate-200 p-6">
        ...
        <form id="filterForm" action="{{ route('ketersediaan.index') }}" method="GET" class="relative z-10 grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-5 items-end">
            <div>
                ...
                <select name="jenis" onchange="this.form.submit()" class="w-full bg-slate-800 text-white border border-slate-700 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-all cursor-pointer">
                    ...
                    <option value="aset" {{ request('jenis') == 'aset' ? 'selected' : '' }}>Aset Tetap</option>
                    <option value="bhp_inventaris" {{ request('jenis') == 'bhp_inventaris' ? 'selected' : '' }}>BHP BPP Inventaris Kantor</option>
                    <option value="bhp_atk" {{ request('jenis') == 'bhp_atk' ? 'selected' : '' }}>BHP ATK Operasional</option>
                </select>
            </div>
            ...
            <div>
                ...
                <select name="status" onchange="this.form.submit()" class="w-full bg-slate-800 text-white border border-slate-700 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-indigo-500 outline-none transition-all cursor-pointer">
                    ...
                    <option value="didistribusikan"   {{ request('status') == 'didistribusikan'   ? 'selected' : '' }}>Telah Didistribusikan</option>
                    <option value="tidak_dipinjamkan" {{ request('status') == 'tidak_dipinjamkan' ? 'selected' : '' }}>Tidak untuk Dipinjamkan</option>
                    <option value="unavailable"       {{ request('status') == 'unavailable'       ? 'selected' : '' }}>Tidak Tersedia (Habis/Non-Aktif)</option>
                </select>
            </div>
                    ...
                    @forelse($barangs as $barang)
                        ...
                        <tr>
                            <td class="px-6 py-5">
                                <div class="flex items-center gap-4">
                                    <div class="w-11 h-11 rounded-2xl flex items-center justify-center shadow-sm 
                                        {{ $isAset ? 'bg-indigo-50 text-indigo-600' : 'bg-orange-50 text-orange-600' }}">
                                        @if($isAset)
                                    ...
                                    <div class="item-info flex flex-col justify-start">
                                        <h4 class="item-name text-sm font-bold text-slate-800 break-words">{{ $barang->nama_barang }}</h4>
                                        <span class="item-code text-sm text-slate-400 font-semibold tracking-wide">#{{ $barang->kode_barang }}</span>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-4 py-5">
                                <div class="flex items-center gap-2">
                                    <span class="relative flex h-2 w-2">
                                        ...
                                        @endif
                                        <span class="relative inline-flex rounded-full h-2 w-2 {{ $tersedia ? 'bg-emerald-500' : 'bg-rose-500' }}"></span>
                                    </span>
                                    <span class="text-sm font-bold {{ $tersedia ? 'text-emerald-600' : 'text-rose-600' }} uppercase tracking-wide">
                                        {{ $statusText }}
                                    </span>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-5 text-center">
                                ...
                                @elseif($kondisiText === 'TELAH DIDISTRIBUSIKAN' || $kondisiText === 'TIDAK UNTUK DIPINJAMKAN')
                                ...
                                @elseif($kondisiText === 'SEDANG DIPINJAM')
                            ...
                            <td class="px-6 py-5 text-center pr-6">
                                @if($kondisiText === 'TERSEDIA')
                                    @if($canBorrow)
                                        <a href="{{ route('peminjaman.index', ['kode_barang' => $barang->kode_barang]) }}" 
                                    ...
                                    @elseif($canUse)
                                        <a href="{{ route('pemakaian.index', ['kode_barang' => $barang->kode_barang]) }}" 
                                    ...
                                    @endif
                                @elseif($kondisiText === 'MENIPIS' && $canUse)
                                    <a href="{{ route('pemakaian.index', ['kode_barang' => $barang->kode_barang]) }}" 
                                    class="inline-flex items-center justify-center min-w-[140px] px-4 py-3 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-bold rounded-2xl transition-all shadow-sm">
                                        Pakai
                                    </a>
