...
<div class="space-y-6 pb-8 animate-main max-w-[1400px]">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div class="flex items-center gap-2.5">
            <a href="{{ route('pemakaian.index') }}" class="text-sm font-semibold text-white bg-sky-400 px-4 py-2.5 rounded-lg shadow-lg shadow-sky-400/25 hover:bg-sky-500 transition-all"><- Kembali ke Form</a>
        </div>
    </div>
    ...
    <div class="modern-card p-5">
        ...
        <form id="filterForm" method="GET" action="{{ route('pemakaian.riwayat') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div class="flex items-end gap-2"><a href="{{ route('pemakaian.riwayat') }}" class="w-full text-center px-4 py-2.5 bg-slate-100 text-slate-700 rounded-xl text-sm font-bold hover:bg-slate-200 transition-all">Reset Filter</a></div>
        </form>
    </div>

    <div class="modern-card p-5">
        <div class="mb-4 flex items-center justify-between"><h3 class="panel-title">Daftar Riwayat ({{ $riwayat->total() }} Item Ditemukan)</h3></div>
        <div class="space-y-3">
                            ...
                            <div class="flex items-start gap-4">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-3 mb-2">
                                        <div class="min-w-0">
                                            <p class="history-code">{{ $item->kode_barang ?? 'N/A' }}</p>
                                            <h4 class="history-title">{{ $item->barang->nama_barang ?? 'Barang Tidak Ditemukan' }}</h4>
                                            <p class="history-note"><strong>Alasan:</strong> {{ $item->alasan_kebutuhan }}</p>
                                        </div>
                                        <span class="status-badge border {{ $statusClass }}">{{ $item->status }}</span>
                                    </div>
                                    <div class="history-meta">
                                        <span><strong>{{ $item->user->name ?? 'USER TIDAK ADA' }}</strong></span>
                                        <span class="history-dot">•</span>
                                        <span><strong>TANGGAL:</strong> {{ \Carbon\Carbon::parse($item->tanggal_pemakaian)->format('d/m/Y') }}</span>
                                        <span class="history-dot">•</span>
                                        <span><strong>JUMLAH:</strong> {{ $item->jumlah }} {{ strtoupper($item->barang->satuan ?? '') }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
