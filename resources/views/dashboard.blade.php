...
@section('page_title', 'Dashboard Pengajuan')
            ...
            <div class="relative z-10">
                ...
                <div>
                    ...
                    <div class="flex items-baseline gap-2">
                        <span class="text-3xl font-black text-brand-dark">{{ $totalAsetCount }}</span>
                        <span class="text-xs font-bold text-slate-400">Unit</span>
                    </div>
                    <p class="mt-2 text-sm font-semibold text-cyan-700">
                        Unit tersedia: {{ number_format($jumlahUnitAset, 0, ',', '.') }} | Aktif: {{ number_format($asetAktif, 0, ',', '.') }}
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="modern-card p-5 group">
            ...
            <div class="relative z-10">
                ...
                <div>
                    ...
                    <div class="flex items-baseline gap-2">
                        <span class="text-3xl font-black text-brand-dark">{{ $totalBHPCount }}</span>
                        <span class="text-xs font-bold text-slate-400">Item</span>
                    </div>
                    <p class="mt-2 text-sm font-semibold text-cyan-700">
                        Stok gudang: {{ number_format($jumlahStokBHP, 0, ',', '.') }} | Menipis/Habis: {{ number_format($bhpMenipisHabis, 0, ',', '.') }}
                    </p>
                </div>
            </div>
        </div>

        {{-- Total Pengajuan Approval --}}
        <div class="modern-card p-5 group">
            ...
            <div class="relative z-10">
                ...
                <div>
                    ...
                    <div class="flex items-baseline gap-2">
                        <span class="text-3xl font-black text-brand-dark">{{ $totalPending }}</span>
                        <span class="text-xs font-bold text-slate-400">Berkas</span>
                    </div>
                    <p class="mt-2 text-sm font-semibold text-amber-700">
                        Peminjaman: {{ number_format($pendingPeminjaman, 0, ',', '.') }} | Pemakaian: {{ number_format($pendingPemakaian, 0, ',', '.') }} | Pembelian: {{ number_format($pendingPembelian, 0, ',', '.') }}
                    </p>
                </div>
            </div>
        </div>
            ...
            <div class="modern-card overflow-hidden">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full modern-table">
                        ...
                        <tbody>
                            @foreach($ketersediaanBarang as $item)
                            @php
                            ...
                            <tr>
                                <td>
                                    <div class="flex items-center gap-3">
                                        <div class="item-category-icon {{ $isAset ? 'is-aset' : 'is-bhp' }}">
                                            @if($isAset)
                                        ...
                                        <div>
                                            <p class="text-sm font-bold text-brand-dark">{{ $item->nama_barang }}</p>
                                            <p class="text-xs font-semibold text-slate-400 uppercase tracking-wide mt-0.5">Kode: {{ $item->kode_barang }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <span class="px-3 py-1.5 rounded-md text-xs font-bold text-slate-600 bg-slate-100">
                                        {{ $item->kategori_barang == 'aset' ? 'Aset Tetap' : 'Habis Pakai' }}
                                    </span>
                                </td>
                                <td class="text-right">
                                    <span class="font-black text-brand-dark text-sm">{{ $item->stok ?? 1 }}</span> 
                                    <span class="text-xs text-slate-400 font-medium">{{ $item->satuan ?? 'Unit' }}</span>
                                </td>
                                <td class="text-center">
                                    <span class="inline-flex px-3 py-1.5 rounded-md text-xs font-bold border {{ $statusClass }}">{{ $statusLabel }}</span>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
        ...
        <div class="space-y-6 min-w-0">
            <div class="modern-card p-6 bg-brand-dark text-white border-none shadow-xl relative overflow-hidden h-full">
                ...
                <div class="relative z-10">
                    ...
                    <div class="space-y-6">
                        @forelse($semuaAktivitas as $log)
                        <div class="flex gap-4 group">
                            <div class="flex flex-col items-center">
                                <div class="w-3 h-3 rounded-full {{ $log['color'] }} shadow-[0_0_12px_rgba(72,202,228,0.6)] border border-brand-dark"></div>
                                <div class="w-0.5 h-full bg-brand-navy/50 my-2 rounded-full"></div>
                            </div>
                            <div class="pb-1">
                                <p class="text-sm text-slate-300 leading-relaxed">
                                    <span class="font-bold text-white">{{ $log['user'] }}</span> {{ $log['aksi'] }} <span class="text-brand-cyan font-bold">{{ $log['objek'] }}</span>
                                </p>
                                <p class="text-xs text-slate-500 mt-1.5 uppercase font-bold tracking-wider">{{ $log['info'] }} • {{ $log['waktu'] }}</p>
                            </div>
                        </div>
                        @empty
                    ...
                    <a href="{{ route('riwayat.index') }}" class="mt-8 flex items-center justify-center w-full py-3 bg-white/5 hover:bg-white/10 rounded-xl text-sm font-bold transition-all border border-white/10 text-brand-light">
                        Lihat Log Lengkap
                    </a>
                </div>
            </div>
