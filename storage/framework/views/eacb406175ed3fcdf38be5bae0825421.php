<?php $__env->startSection('title', 'Peminjaman Barang Kantor'); ?>
<?php $__env->startSection('page_title', 'Form Peminjaman Barang Kantor'); ?>
...
<?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(isset($notifikasi) && $notifikasi): ?>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2">
            <div class="modern-card p-6">
                <form action="<?php echo e(route('peminjaman.store')); ?>" method="POST" enctype="multipart/form-data" id="peminjamanForm" class="space-y-5" novalidate>
                    <?php echo csrf_field(); ?>
                    
                    <div>
                        <label class="modern-label">Nama Pengaju</label>
                        <input type="text" class="modern-input px-3 py-2.5 bg-slate-100 cursor-not-allowed" value="<?php echo e(Auth::user()->name); ?> (<?php echo e(Auth::user()->user_group); ?>)" readonly>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            ...
                            <input type="date" name="tanggal_pinjam" id="tanggal_pinjam" class="modern-input px-3 py-2.5" min="<?php echo e(date('Y-m-d')); ?>" value="<?php echo e(date('Y-m-d')); ?>" required>
                        </div>
                        <div>
                            <label class="modern-label">Tanggal Pengembalian</label>
                            <input type="date" name="tanggal_pengembalian" id="tanggal_pengembalian" class="modern-input px-3 py-2.5" min="<?php echo e(date('Y-m-d')); ?>" value="<?php echo e(date('Y-m-d')); ?>" required>
                        </div>
                    </div>
                                        ...
                                        <select name="items[0][kode_barang]" class="barang-select barang-select-native modern-input px-3 py-2.5 bg-white">
                                            ...
                                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php $__currentLoopData = $barangs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $barang): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                <option value="<?php echo e($barang->kode_barang); ?>" data-stok="<?php echo e($barang->stok); ?>" <?php echo e(old('items.0.kode_barang', request('kode_barang')) == $barang->kode_barang ? 'selected' : ''); ?>>
                                                    <?php echo e($barang->kode_barang); ?> - <?php echo e($barang->nama_barang); ?> (<?php echo e($barang->jenis_barang_label); ?>)
                                                </option>
        ...
        <div class="space-y-6">
            <div class="modern-card p-5">
                ...
                <div id="filterForm" class="mb-6 p-4 bg-blue-50/50 rounded-2xl border border-blue-100 <?php echo e(request()->anyFilled(['tanggal_dari', 'tanggal_sampai', 'status']) ? '' : 'hidden'); ?>">
                    <form method="GET" action="<?php echo e(route('peminjaman.index')); ?>" class="space-y-3">
                        ...
                        <div class="flex gap-2 pt-1">
                            ...
                            <a href="<?php echo e(route('peminjaman.index')); ?>" class="px-3 py-2.5 bg-white text-slate-400 rounded-lg text-sm border border-slate-200">Reset</a>
                        </div>
                    </form>
                </div>
                    ...
                    <div class="p-4 rounded-2xl border border-slate-50 bg-slate-50/30 hover:bg-white hover:border-blue-100 hover:shadow-md transition-all group">
                        <div class="flex justify-between items-start mb-2">
                            <div class="space-y-1">
                                ...
                                <p class="text-xs font-bold text-slate-700">x<?php echo e($item->jumlah_pinjam); ?> Unit</p>
                            </div>
                            <?php
                            ...
                            <span class="status-badge <?php echo e($statusColor); ?>">
                                <?php echo e(match($item->status_pinjam) {
                        ...
                        <div class="flex items-center justify-end text-xs text-slate-500 pt-3 border-t border-slate-100/50">
                            <span class="flex items-center gap-1"><i class="far fa-calendar"></i> <?php echo e(\Carbon\Carbon::parse($item->tanggal_pinjam)->format('d/m/y')); ?></span>
                        </div>
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($item->status_pinjam == 'disetujui'): ?>
                        <form action="<?php echo e(route('peminjaman.kembalikan', $item->id_peminjaman)); ?>" method="POST" enctype="multipart/form-data" class="mt-3 space-y-2">
                            <?php echo csrf_field(); ?>
                        ...
                        <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($item->status_pinjam == 'menunggu_verifikasi_pengembalian'): ?>
                ...
                <div class="mt-6 pt-4 border-t border-slate-100">
                    <a href="<?php echo e(route('peminjaman.riwayat')); ?>" class="block text-center py-3 bg-sky-400 hover:bg-sky-500 text-white rounded-xl text-xs font-bold shadow-lg shadow-sky-400/25 transition-all">
                        Buka Semua Riwayat
                    </a>
                </div>
                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
...
document.addEventListener('DOMContentLoaded', function() {
    let itemIndex = 1;
    const barangOptions = `<?php echo $barangs->map(fn($b) => '<option value="' . e($b->kode_barang) . '" data-stok="' . e($b->stok) . '">' . e($b->kode_barang . ' - ' . $b->nama_barang . ' (' . $b->jenis_barang_label . ')') . '</option>')->implode(''); ?>`;
...
<?php echo $__env->make('layouts.sidebar', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/project/resources/views/peminjaman-barang/index.blade.php ENDPATH**/ ?>