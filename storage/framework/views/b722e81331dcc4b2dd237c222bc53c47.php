<?php $__env->startSection('title', 'Semua Riwayat Pembelian'); ?>
<?php $__env->startSection('page_title', 'Riwayat Pengajuan Pembelian'); ?>
...
<div class="space-y-6 pb-8 animate-main max-w-[1400px]">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <a href="<?php echo e(route('pembelian.index')); ?>" class="text-sm font-semibold text-white bg-sky-400 px-4 py-2.5 rounded-lg shadow-lg shadow-sky-400/25 hover:bg-sky-500 transition-all"><- Kembali ke Form</a>
    </div>
    ...
    <div class="modern-card p-5">
        ...
        <form method="GET" action="<?php echo e(route('pembelian.riwayat')); ?>" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div><label class="modern-label">Tanggal Dari</label><input type="date" name="tanggal_dari" value="<?php echo e(request('tanggal_dari')); ?>" class="modern-input px-3 py-2 w-full" onchange="this.form.submit()"></div>
            <div><label class="modern-label">Tanggal Sampai</label><input type="date" name="tanggal_sampai" value="<?php echo e(request('tanggal_sampai')); ?>" class="modern-input px-3 py-2 w-full" onchange="this.form.submit()"></div>
            <div>
            ...
            <div class="flex items-end"><a href="<?php echo e(route('pembelian.riwayat')); ?>" class="w-full text-center px-4 py-2.5 bg-slate-100 text-slate-700 rounded-xl text-sm font-bold hover:bg-slate-200">Reset</a></div>
        </form>
    </div>

    <div class="modern-card p-5">
        <div class="mb-4 flex items-center justify-between"><h3 class="panel-title">Daftar Pembelian (<?php echo e($riwayat->total()); ?> Item Ditemukan)</h3></div>
        <div class="space-y-3">
                ...
                <div class="history-item">
                    <div class="flex flex-col xl:flex-row xl:items-start xl:justify-between gap-5">
                        <div class="flex gap-4 flex-1 min-w-0">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-3 mb-2">
                                    <div class="min-w-0">
                                        <span class="inline-flex bg-slate-100 text-slate-500 px-2.5 py-1 rounded-md text-xs font-black uppercase"><?php echo e($item->kategori_barang); ?></span>
                                        <h4 class="history-title mt-1"><?php echo e($item->nama_barang); ?></h4>
                                        <p class="history-note">Oleh: <?php echo e($item->user->name); ?> | <?php echo e($item->jumlah); ?> Unit</p>
                                    </div>
                                </div>
                                <div class="history-meta">
                                    <span><strong>PEMOHON:</strong> <?php echo e($item->user->name); ?></span>
                                    <span class="history-dot">•</span>
                                    <span><strong>TANGGAL:</strong> <?php echo e(\Carbon\Carbon::parse($item->tanggal_pengajuan)->format('d/m/Y')); ?></span>
                                    <span class="history-dot">•</span>
                                    <span><strong>JUMLAH:</strong> <?php echo e($item->jumlah); ?> UNIT</span>
                                </div>
                            </div>
                        </div>
                        <div class="text-left xl:text-right xl:self-center">
                            ...
                            <p class="text-2xl font-black text-blue-600 mt-3">Rp <?php echo e(number_format($item->sub_total, 0, ',', '.')); ?></p>
                        </div>
                    </div>
                </div>
...
<?php echo $__env->make('layouts.sidebar', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/project/resources/views/pembelian/riwayat.blade.php ENDPATH**/ ?>